"""
Train YeastCNN (src/cnn.py) on the frame-split dataset (src/dataset.py).

Mini-batch SGD via Adam, early stopping on validation loss (patience-based,
restore best weights), then a full evaluation on the held-out test split --
including agreement/disagreement against BOTH the classical zero-crossing
labels and the modern convexity-defect labels the model was trained on.
"""
import copy
import sys
import time
...
from cnn import YeastCNN, Adam, bce_with_logits
from dataset import load_split, augment_batch, INPUT_SIZE
...
def snapshot(model):
    return copy.deepcopy({l: {"W": layer.W.copy(), "b": layer.b.copy()}
...
def main():
    ...
    Xtest, ytest, meta_test = load_split("test")
    print(f"train {Xtr.shape}, val {Xval.shape}, test {Xtest.shape}")

    model = YeastCNN(input_size=INPUT_SIZE, seed=SEED)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        for xb, yb in iterate_batches(Xtr, ytr, BATCH_SIZE, rng):
            if USE_AUGMENTATION:
        ...
        val_metrics = evaluate(model, Xval, yval)
    ...
    test_metrics = evaluate(model, Xtest, ytest)
    ...
    # save weights
    out = Path(__file__).parent.parent / "data" / "cnn_weights.npz"
    np.savez(out,
    ...
    # write per-test-crop predictions for later analysis (classical vs modern vs CNN)
    import csv
    pred_path = Path(__file__).parent.parent / "data" / "cnn_test_predictions.csv"
